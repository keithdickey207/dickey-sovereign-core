...
def transduction_layer_audit() -> list[dict]:
    return [
        {"method": "Piezoresistive (FSR)", "strength": "Absolute grip force", "weakness": "Low spatial resolution", "qctb_encoding": "ΔR → RY rotation angle", "critical_path": True},
        {"method": "Capacitive array", "strength": "Localized pressure map C ∝ 1/d", "weakness": "Matrix scan latency at high density", "qctb_encoding": "ΔC vector → amplitude encoding", "critical_path": True},
        {"method": "Piezoelectric", "strength": "Dynamic slip/texture (1 kHz events)", "weakness": "No static force", "qctb_encoding": "Event triggers + stochastic VQC branch", "critical_path": True},
        {"method": "Optical (GelSight)", "strength": "Rich 3D contact mesh", "weakness": "Higher compute cost", "qctb_encoding": "CV features → quantum kernel SVM", "critical_path": False},
    ]
...
def print_audit(report: dict):
    ...
    sp, tp = topo["spatial_path"], topo["temporal_path"]
    print(f"\n[1] Spatial flatten: spec={sp['flatten_spec']} actual={sp['flatten_actual']}")
    print(f"[2] Temporal flatten: spec={tp['flatten_spec']} actual={tp['flatten_actual']}")
    p = report["corrected_tactile_net"]
    print(f"[3] Parameters: {p['total_params']:,}")
    lat = report["latency_budget"]
    print(f"[4] WCET total: {lat['critical_path_total_ms']:.2f} ms")
    print("=" * 68 + "\n")
...
def main():
    ...
    parser.add_argument("--export-ansatz", metavar="FILE")
